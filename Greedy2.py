...
import signal
import time
from plot import  plot_training_metrics, plot_training_dashboard, plot_reward_comparison, \
...
SAVE_DIR = "/home/dc/data_new/ppo_results/greedy/2"
...
def main():
    ...
    greedy_trainer = GreedySearchBaseline(
        env,
        DEVICE,
        baseline_config
    )

    greedy_df, greedy_reward_means = greedy_trainer.run()

    print("Greedy baseline finished.")

    # ---------------- CEM ----------------

    cem_trainer = CEMBaseline(
        env,
        DEVICE,
        baseline_config
    )

    cem_df, cem_reward_means = cem_trainer.run()

    print("CEM baseline finished.")

    print("\nAll control groups finished.")
