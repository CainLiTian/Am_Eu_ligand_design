...
def create_var(tensor, requires_grad=None, device=device):  # 添加设备参数
    """将张量移动到指定设备并封装为 Variable"""
    if not isinstance(tensor, torch.Tensor):
        tensor = torch.tensor(tensor)  # 确保输入为张量

    # 统一设备分配
    var_tensor = tensor.to(device)

    # 设置梯度要求
    if requires_grad is not None:
        var = torch.autograd.Variable(var_tensor, requires_grad=requires_grad)
    else:
        var = torch.autograd.Variable(var_tensor)
...
def avg_pool(all_vecs, scope, dim, device=None):
    size = create_var(torch.tensor([le for _, le in scope], dtype=all_vecs.dtype))
